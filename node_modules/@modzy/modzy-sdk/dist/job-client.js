...
class JobClient {
  ...
  */
  ...
  getJobHistory(user, accessKey, startDate, endDate, model, status, page, perPage = 1000, direction, sortBy) {
  ...
  submitJobFiles(modelId, versionId, fileSources) {
    ...
    return this.submitJob({
      ...
      Object.keys(fileSources).forEach(inputItemKey => {
        Object.keys(fileSources[inputItemKey]).forEach(dataItemKey => {
      ...
      return this.cancelJob(job.jobIdentifier).then(_ => {
        throw apiError;
      }).catch(_ => {
        throw apiError;
      });
  ...
  submitJobEmbedded(modelId, versionId, mediaType, embeddedSources) {
    let encodedSources = {};
    Object.keys(embeddedSources).forEach(sourceKey => {
      let source = {};
      Object.keys(embeddedSources[sourceKey]).forEach(key => {
        source[key] = "data:" + mediaType + ";base64," + Buffer.from(embeddedSources[sourceKey][key], 'binary').toString('base64');
  ...
  blockUntilComplete(job) {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        this.getJob(job.jobIdentifier).then(updatedJob => {
  ...
  appendInputChunk(requestURL, asyncGenerator, chunkSize, dataItemKey, chunkCount) {
    return asyncGenerator.next().then(entry => {
