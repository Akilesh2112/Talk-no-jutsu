...
// get voices ( Show the list of available voice to the user ) 
...
function lipsync( arg ) {
    if ( arg == ' ' || arg == '.' ) {
        botmouth.innerHTML = oldmouth;
    }
    if ( arg == 'a' ) {
        botmouth.innerHTML = 'D';
    }
    if ( arg == 'b' ) {
        botmouth.innerHTML = '|';
    }
    ...
    if ( arg == 'j' ) {
        botmouth.innerHTML = 'I';
    }
    if ( arg == 'k' ) {
        botmouth.innerHTML = 'I';
    }
    if ( arg == 'l' ) {
        botmouth.innerHTML = 'P';
    }
    if ( arg == 'm' ) {
        botmouth.innerHTML = '(';
    }
...
function blink() {
    ...
    eyeL.innerHTML = '-';
    setTimeout( function () {
        eyeR.innerHTML = oldeye;
        eyeL.innerHTML = oldeye;
    }, 200 );
}
...
window.num2str.teens = [ 'ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen',
    'eighteen', 'nineteen'
];
...
window.num2str.convert_millions = function ( num ) {
    if ( num >= 1000000 ) {
        return this.convert_millions( Math.floor( num / 1000000 ) ) + " million " + this.convert_thousands(
            num % 1000000 );
    } else {
...
window.num2str.convert_thousands = function ( num ) {
    if ( num >= 1000 ) {
        return this.convert_hundreds( Math.floor( num / 1000 ) ) + " thousand " + this.convert_hundreds(
            num % 1000 );
    } else {
...
b.a = b.appendChild;
p = c.width = c.height = 400, z = b.a( kx.c( "p" ) ), E = a.createImageData( p, p ), q = E.data, D = j.value = j
    .max = 10, g = 190, n = 200, w = g * g, o = 255, e = D, f = -e, r = 16040, l = Math, u = l.PI, B = 2 * u,
    C = l.sqrt, s = l.atan2;
